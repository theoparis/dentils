...
export class PrefixLogger extends BaseLogger {
    ...
    constructor(public prefix: string, base?: ILogger) {
        super(base);
    }
    ...
    debug(message?: string, ...optionalParams: unknown[]): void {
        super.debug(ink.colorize(`<blue>${this.prefix}</blue> > ${message}`));
    }
    info(message?: string, ...optionalParams: unknown[]): void {
        super.info(ink.colorize(`<green>${this.prefix}</green> > ${message}`));
    }
    ...
    error(message?: string, ...optionalParams: unknown[]): void {
        super.error(ink.colorize(`<red>${this.prefix}</red>${message}`));
    }
}
